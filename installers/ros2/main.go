...
func (i Installer) BootScript() job.BootScript {
	return func(ctx context.Context, j job.Job, s ipxe.Script) ipxe.Script {
		...
		s.Kernel("${url}/rancheros-${version}-${arch}-kernel")
		s.Args("ip=dhcp", "rd.cos.disable", "root=live:${url}/${rootfs}",
			"rancheros.install.automatic=true", "rancheros.install.iso_url=${url}/${iso}",
			"console=tty1")
		s.Args(j.UserData())
		s.Initrd("${url}/rancheros-${version}-${arch}-initrd")
		s.Boot()
		j.DisablePXE(ctx)
		return s
	}
